...
func (h *SafetyIncidentHandler) OverdueAcceptance(c *gin.Context) {
	list, err := h.svc.OverdueAcceptanceQueue()
	if err != nil {
		h.wrapError(c, err, "SafetyIncident overdue acceptance queue failed")
		return
	}
...
func (h *SafetyIncidentHandler) Supervise(c *gin.Context) {
	...
	sup, err := h.svc.Supervise(id, middleware.GetUserID(c), req.Note)
	if err != nil {
		h.wrapError(c, err, "SafetyIncident supervise failed")
		return
	}
	OKWithMessage(c, constants.MsgIncidentSupervised, sup)
}
